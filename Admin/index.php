...
<head>
    ...
    <style>
        /* CSS TAMBAHAN UNTUK MENU NAVIGASI */
        .admin-nav {
            display: grid;
...
<body>
    ...
    <div class="admin-nav">
        <a href="index.php" class="nav-card active">
        ...
        <a href="pesanan.php" class="nav-card">
    ...
    <div class="dashboard-grid">
        <div class="glass-panel">
            <h3><i class="fas <?php echo $edit_data ? 'fa-edit' : 'fa-plus-circle'; ?>"></i> <?php echo $edit_data ? 'Edit Game' : 'Tambah Game Baru'; ?></h3>
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nama_game" value="<?php echo $edit_data['nama_game'] ?? ''; ?>" required placeholder="Contoh: Mobile Legends">
                </div>

                <div class="form-group">
                    <label>Slug URL (Huruf kecil, tanpa spasi)</label>
                    <input type="text" name="slug" value="<?php echo $edit_data['slug'] ?? ''; ?>" required placeholder="Contoh: mobile-legends">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="tipe_input" style="width:100%; padding:12px; background:rgba(0,0,0,0.3); border:1px solid var(--glass-border); color:white; border-radius:10px;">
                        <option value="id_server" <?php echo ($edit_data['tipe_input'] ?? '') == 'id_server' ? 'selected' : ''; ?>>ID & Server (MLBB, dll)</option>
                        <option value="id_only" <?php echo ($edit_data['tipe_input'] ?? '') == 'id_only' ? 'selected' : ''; ?>>ID Only (FF, Roblox, dll)</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>Banner Game (Format JPG/PNG)</label>
                    <input type="file" name="gambar" accept="image/*" id="imgInput">
                </div>

                <button type="submit" name="submit" class="btn btn-primary"><?php echo $edit_data ? 'Update Data' : 'Simpan Game'; ?></button>
                ...
                <table>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($query)) : ?>
                            <tr>
                                <td><img src="../img/<?php echo $row['gambar_banner']; ?>" width="50" style="border-radius:5px; aspect-ratio:1/1; object-fit:cover;"></td>
                                <td>
                                ...
                                <td>
                                    <?php if ($row['is_popular'] == 1): ?>
                                        <span style="color:#ff4500; font-weight:bold; font-size:0.8rem;">🔥 Hot</span>
                                    <?php else: ?>
                                        <span style="color:#888; font-size:0.8rem;">Normal</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <a href="?edit=<?php echo $row['id']; ?>" class="action-btn edit-btn"><i class="fas fa-pen"></i></a>
                                    <a href="?hapus=<?php echo $row['id']; ?>" class="action-btn del-btn" onclick="return confirm('Yakin mau hapus game ini?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
